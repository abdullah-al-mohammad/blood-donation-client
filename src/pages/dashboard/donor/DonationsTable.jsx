...
const DonationsTable = ({ donation, refetch }) => {
        ...
        <div className="flex items-center gap-3">
          <div>
            <div className="font-bold">
              {status === 'inprogress' || status === 'done' || status === 'cancelled'
                ? recipientName
                : 'inprogress'}
            </div>
          </div>
        ...
        <details className="dropdown">
          ...
          {status === 'done' || status === 'cancelled' ? (
            ...
            <ul className="menu dropdown-content bg-base-100 rounded-box z-1 w-32 p-2 shadow-sm">
              <li>
                <button onClick={() => handleUpdateStatus('inprogress')} type="button">
                  Inprogress
                </button>
              </li>
              {status === 'inprogress' ? (
                <>
                  <li>
                    <button onClick={() => handleUpdateStatus('done')} type="button">
                      Done
                    </button>
                  </li>
                  <li>
                    <button onClick={() => handleUpdateStatus('cancelled')} type="button">
                      Cancelled
                    </button>
                  </li>
                </>
                ...
                <li>
                  <button onClick={() => handleUpdateStatus('pending')} type="button">
                    Pending
                  </button>
                </li>
              )}
              <li>
                <Link to={`/dashboard/updateDonationRequest/${_id}`}>Edit</Link>
              </li>
              <li>
