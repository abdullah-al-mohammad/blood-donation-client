      ...
      <div className="pt-16">
        <div className="text-center mb-10">
          ...
          <p className="md:text-2xl">
            We found <span className="text-bold_red-0">{results.length}</span> items matching your
            search
          </p>
        </div>
            ...
            <ul className="max-w-screen-xl">
              {results.map((result, index) => (
                <li key={index} className="border p-2 rounded-md shadow-sm">
                  ...
                  <span className="text-bold_red-0">{result.bloodGroup}</span>
                  <strong className="ml-3">District : </strong>
                  <span className="text-bold_red-0">{result?.district?.value}</span>
                  <strong className="ml-3">Sub-District : </strong>
                  <span className="text-bold_red-0">{result?.subDistrict?.value}</span>
                </li>
              ))}
            </ul>
          ) : (
